...
export function ModalShell({ title, description, onClose, children, widthClassName = 'max-w-4xl' }) {
  ...
  const dialog = (
    <div className="fixed inset-0 z-[60] m-0 flex items-end justify-center overflow-y-auto bg-slate-950/80 backdrop-blur-sm sm:items-start">
      <Card
        className={`theme-panel-raised m-0 mt-0 flex w-full ${widthClassName} max-h-[92dvh] flex-col overflow-hidden rounded-t-[28px] shadow-[0_32px_96px_rgba(15,23,42,0.35)] sm:max-h-[100vh] sm:rounded-[28px]`}
      >
        <div className="theme-divider flex items-start justify-between gap-4 border-b px-5 py-4">
          <div>
        ...
        <div className="min-h-0 flex-1 overflow-y-auto bg-[rgb(var(--panel-2)/0.48)] px-5 py-4">{children}</div>
      </Card>
    </div>
  );
