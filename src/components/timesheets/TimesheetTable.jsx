...
export function TimesheetTable({
  ...
  const selectionSet = useMemo(() => new Set(selectedIds.map(String)), [selectedIds]);
  const updatingSet = useMemo(() => new Set(billableUpdatingIds.map(String)), [billableUpdatingIds]);
  const allSelected = useMemo(() => rows.length > 0 && rows.every((row) => selectionSet.has(String(row.id || row._id))), [rows, selectionSet]);
  const someSelected = useMemo(() => rows.some((row) => selectionSet.has(String(row.id || row._id))), [rows, selectionSet]);
      ...
      ? [
          {
            ...
            render: (row) => (
              <SelectionCheckbox
                checked={selectionSet.has(String(row.id || row._id))}
  ...
  }, [allSelected, canDelete, canEditBillable, onDelete, onToggleAll, onToggleBillable, onToggleRow, selectionSet, showSelectionColumn, someSelected, updatingSet]);
  ...
  return (
    <DataTable
      ...
      rowClassName={(row) => (selectionSet.has(String(row.id || row._id)) ? 'bg-sky-50/70' : '')}
    />
  );
}
