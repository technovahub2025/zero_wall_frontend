...
import { Search, Filter, Users, AlertCircle, Clock3, ListTodo } from 'lucide-react';
      ...
      <div className="rounded-3xl border border-[rgb(var(--line)/0.16)] bg-[rgb(var(--panel-2)/0.82)] p-4 shadow-sm backdrop-blur">
        <div className="flex flex-wrap items-center gap-3">
          ...
          <div className="ml-auto flex flex-wrap gap-2">
            ...
            <MiniStat label="Critical" value={stats.critical} icon={Filter} tone="amber" />
          </div>
        </div>
          ...
          <div className="flex min-w-max gap-4 px-1 pr-4">
            {BOARD_COLUMNS.map((column) => (
              <KanbanColumn
                ...
                onAdd={(payload) => createTask.mutateAsync({ ...payload, project: project?.id })}
                addCardProps={{
                  employees,
